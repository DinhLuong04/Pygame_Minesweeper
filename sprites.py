...
class Tile:
    ...
    def __repr__(self):
        return f"Tile(x={self.x}, y={self.y}, type={self.type}, revealed={self.revealed}, flagged={self.flagged})"
...
class Board:
    ...
    def dig(self, x, y):
        ...
        if self.board_list[x][y].type == "X":
            ...
            return False  # Exploded, game over
        ...
        for row in range(max(0, x - 1), min(ROWS - 1, x + 1) + 1):
            for col in range(max(0, y - 1), min(COLS - 1, y + 1) + 1):
                if (row, col) not in self.dug:
                    self.dig(row, col)  # Recursive dig
